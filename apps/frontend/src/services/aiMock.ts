// ThinkComma - Mock AI Service helpers (kept separate from API client)
...
type ChoiceLabelSet = { labelA: string; labelB: string };
type RawChoice = { id: 'A' | 'B'; content: string; description?: string };
type RawQuestion = { text: string; choices: RawChoice[] };

const MAX_QUESTIONS = 10;

const getBaseQuestionsByCategory = (category?: string): RawQuestion[] => {
  const questionSets: Record<string, RawQuestion[]> = {
    career: [
      {
  ...
  const commonQuestions: RawQuestion[] = [
    {
...
const generateContextSpecificQuestions = (
  worryContent: string
): RawQuestion[] => {
  const keywords = worryContent.toLowerCase();
  const contextQuestions: RawQuestion[] = [];
...
const buildQuestions = (worry: WorryInput): Question[] => {
  const base = getBaseQuestionsByCategory(worry.category);
  const context = generateContextSpecificQuestions(worry.content);
  const rawQuestions = [...context, ...base].slice(0, MAX_QUESTIONS);

  return rawQuestions.map((q, index) => ({
    id: index + 1,
    text: q.text,
    choices: [q.choices[0], q.choices[1]] as [RawChoice, RawChoice],
  }));
};

const getChoiceLabels = (category?: string): ChoiceLabelSet => {
  const defaultLabels = {
    labelA: '현재 방식 유지하기',
    labelB: '적극적인 변화 추구하기',
  };

  const categoryLabels: Record<string, ChoiceLabelSet> = {
    career: {
...
const generatePersonalityTraits = (
  ...
  const stressLevel = Math.random() * 40 + 30;
  const decisionSpeed = Math.random() * 50 + 25;
  ...
  return [
    ...
    },
    {
      ...
      level: stressLevel >= 60 ? 'high' : stressLevel >= 40 ? 'medium' : 'low',
    },
    {
      name: '결정력',
...
const generateDecisionFactors = (
  ...
  return [
    ...
    {
      name: '장기적 만족도',
      score: Math.round(changeNeed * 0.9),
...
const generateActionSteps = (
  ...
  const actionStepsByCategory: Record<string, { A: string[]; B: string[] }> = {
    career: {
      A: [
...
const generateSummary = (recommendedChoice: 'A' | 'B'): string => {
  const summaries = {
...
const buildFramingIntro = (
  worry: WorryInput,
  labels: ChoiceLabelSet
): FramingIntro => ({
  summary: `이 고민은 "${labels.labelA}" vs "${labels.labelB}"로 정리할 수 있어요.`,
  choiceALabel: labels.labelA,
  choiceBLabel: labels.labelB,
  aHint: '현재 상황을 유지하면서 개선하는 방향을 떠올려보세요.',
  bHint: '변화를 통해 얻을 수 있는 이점을 상상해보세요.',
  cta: '이렇게 설정해서 10개 질문으로 분석해볼까요?',
});

const buildAnalysisResult = (
  worry: WorryInput,
  responses: UserResponse[],
  labelsOverride?: { choiceALabel?: string; choiceBLabel?: string }
): AnalysisResult => {
  const aCount = responses.filter(r => r.selectedChoice === 'A').length;
  const bCount = responses.filter(r => r.selectedChoice === 'B').length;
  const total = Math.max(responses.length, 1);

  const scoreA = Math.round(aCount / total);
  const scoreB = Math.round(bCount / total);
  const recommendedChoice: 'A' | 'B' = scoreB > scoreA ? 'B' : 'A';
  const confidence = Math.max(scoreA, scoreB);

  const baseLabels = getChoiceLabels(worry.category);
  const labelA = labelsOverride?.choiceALabel ?? baseLabels.labelA;
  const labelB = labelsOverride?.choiceBLabel ?? baseLabels.labelB;

  return {
    recommendedChoice,
    recommendedChoiceLabel: recommendedChoice === 'A' ? labelA : labelB,
    otherChoiceLabel: recommendedChoice === 'A' ? labelB : labelA,
    confidence,
    scoreA,
    scoreB,
    personalityTraits: generatePersonalityTraits(responses),
    decisionFactors: generateDecisionFactors(responses),
    actionSteps: generateActionSteps(worry.category, recommendedChoice),
    summary: generateSummary(recommendedChoice),
    responses,
  };
};

export const aiMockService = {
  generateQuestions: async (worry: WorryInput): Promise<Question[]> =>
    buildQuestions(worry),
  generateFraming: async (worry: WorryInput): Promise<FramingIntro> =>
    buildFramingIntro(worry, getChoiceLabels(worry.category)),
  generateAnalysis: async (
    worry: WorryInput,
    _questions: Question[],
    responses: UserResponse[],
    labels?: { choiceALabel?: string; choiceBLabel?: string }
  ): Promise<AnalysisResult> => buildAnalysisResult(worry, responses, labels),
};
