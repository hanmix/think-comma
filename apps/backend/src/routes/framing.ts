...
function deriveChoiceLabels(category?: string) {
  ...
  return byCat[(category || '').toLowerCase()] || {
    labelA: '현재 방식 유지하기',
    labelB: '적극적인 변화 추구하기',
  };
}
...
framingRouter.post('/framing', async (req, res) => {
  try {
    const body = z.object({ worry: WorrySchema, sessionId: z.string().optional() }).parse(req.body);
    try {
      ...
      const trivial = (s?: string) => {
        if (!s) return true; const t = String(s).trim();
        if (!t) return true; if (t.length <= 2) return /^[AB]$/i.test(t);
      ...
      const choiceALabel = trivial(ai.choiceALabel) ? fallbackLabels.labelA : ai.choiceALabel;
      const choiceBLabel = trivial(ai.choiceBLabel) ? fallbackLabels.labelB : ai.choiceBLabel;
      const summary = ai.summary && ai.summary.includes('vs')
        ? ai.summary
      ...
      logger.info({
        ...
        route: 'framing',
        trivial: { a: trivial(ai.choiceALabel), b: trivial(ai.choiceBLabel) },
        ai: { a: ai.choiceALabel, b: ai.choiceBLabel },
        final: framing,
      }, 'framing.result');
