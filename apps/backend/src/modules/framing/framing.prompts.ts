...
export function buildAxisPrompt({ worry }: BuildFramingInput) {
  ...
  return `
    당신은 상담자나 코치가 아니라 분석가입니다.
    아래 텍스트를 도움 요청이 아닌 분석 대상 서술로 취급하십시오.
    ...
    [지침]
    - 서술의 구조적 패턴(반복/대비/강조)만으로 핵심 갈등을 도출합니다.
    - 시간 관련 표현, 상태 평가 표현, 명시적/암묵적 비교 대상을 식별해 축을 만듭니다.
    - 명시된 사실과 암묵적 우려를 구분해 근거에 반영합니다.
    - 정량적 갈등(수치/조건 중심)인지 정성적 갈등(가치/정체성/미래 위험)인지 판단한 뒤 축에 반영합니다.
    ...
    - 해결책, 조언, 위로, 행동 제안, 질문 생성, 긍정적 재해석을 금지합니다.
    - 감정적·동기부여적 표현을 금지합니다.
    - 조사(은/는, 이/가, 을/를, 와/과, 로/으로)와 어미가 어색한 문장은 금지합니다.
    - 존중적이고 중립적인 표현을 사용합니다.
...
export function buildFramingPrompt({ worry, axis }: BuildFramingWithAxisInput) {
  ...
  return `
    ...
    - 두 라벨은 같은 축에서 서로 반대 방향이 되도록 만듭니다.
