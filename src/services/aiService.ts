...
class AIService {
  ...
  async generateQuestions(worry: WorryInput): Promise<Question[]> {
    ...
    // Generate context-aware questions based on worry category and content
    const baseQuestions = this.getBaseQuestionsByCategory(worry.category);
    const contextQuestions = this.generateContextSpecificQuestions(
      worry.content
    );

    // Merge and customize questions
    const allQuestions = [...baseQuestions, ...contextQuestions];

    // Return first 10 questions
    return allQuestions.slice(0, 10).map((q, index) => ({
      id: index + 1,
      text: q.text,
      choices: q.choices,
    }));
  ...
  async generateAnalysis(
    ...
    const analysisData = this.analyzeResponses(worry, responses);

    return {
      recommendedChoice: analysisData.recommendedChoice,
      recommendedChoiceLabel: analysisData.recommendedChoiceLabel,
      otherChoiceLabel: analysisData.otherChoiceLabel,
      confidence: analysisData.confidence,
      scoreA: analysisData.scoreA,
      scoreB: analysisData.scoreB,
      personalityTraits: analysisData.personalityTraits,
      decisionFactors: analysisData.decisionFactors,
      actionSteps: this.generateActionSteps(
        worry,
        analysisData.recommendedChoice
      ),
      summary: this.generateSummary(worry, analysisData.recommendedChoice),
      responses: responses,
    };
  }
